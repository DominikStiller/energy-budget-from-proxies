...
def convert_datetime_to_decimal_year(da: xr.DataArray, equal_month_lengths=False) -> np.array:
    if equal_month_lengths:
        ...
        return cfr.utils.datetime2year_float(da.data, resolution="day")
...
def use_monthly_npdatetime_time_coords(da: T_Xarray) -> T_Xarray:
    da = da.assign_coords(time=da["time"].astype("datetime64[M]"))
    if "time_bnds" in da:
        da["time_bnds"] = da["time_bnds"].astype("datetime64[M]")
    return da
...
def format_tuple_time(time: float | tuple, year_first=True):
    if isinstance(time, tuple):
        year, season = time
    else:
        year = int(time)
        season = map_decimal_to_season(time)
    if year_first:
        return f"{year}-{season.name}"
    else:
        return f"{season.name} {year}"
