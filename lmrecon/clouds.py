...
from lmrecon.grid import regrid_pressure
...
def calculate_eis(
    psfc: xr.DataArray, Tsfc: xr.DataArray, T700: xr.DataArray, RHsfc: xr.DataArray | float = 80
) -> xr.Dataset:
    # Adapted from https://github.com/zacespinosa/tropical_pacific_clouds/blob/main/EIS_calculations.ipynb
    """
    Calculate Estimated Inversion Strength (EIS) as defined in Wood & Bretherton (2006).
    ...
    Returns:
        EIS
    """
    from metpy.calc import potential_temperature, saturation_mixing_ratio
    from metpy.constants import Cp_d, Lv, Rd, Rv, g
    from metpy.units import units

    psfc = psfc * units.Pa
    p700 = 700 * units.hPa
    p850 = 850 * units.hPa
    RHsfc = RHsfc * units.dimensionless
    Tsfc = Tsfc * units.K

    if "lev" in T700.dims:
        T700 = regrid_pressure(T700.to_dataset(name="T"), "T", [700e2]).isel(lev=0)
    T700 = T700 * units.K

    LTS = potential_temperature(p700, T700) - potential_temperature(psfc, Tsfc)

    LTS = LTS.where(psfc >= p700, 0.0)
    LTS = LTS.where(~np.isnan(LTS), 0.0)
    LTS = LTS.where(LTS >= 0.0, 0.0)

    # Assume isothermal atmosphere with T = Tsfc
    z700 = (Rd * Tsfc / g) * np.log(psfc / p700)

    # plcl, _ = lcl(psfc, Tsfc, dewpoint_from_relative_humidity(Tsfc, RHsfc / 100))
    # zlcl = (Rd * Tsfc / g) * np.log(psfc / plcl)
    # Approximation from https://doi.org/10.1175/BAMS-86-2-225, Eq. 24
    zlcl = (20 + (Tsfc.pint.to("K").pint.dequantify() - 273.15) / 5.0) * (100 - RHsfc) * units.m

    T850 = (Tsfc + T700) / 2.0

    qs850 = saturation_mixing_ratio(p850, T850)
    gamma_num = 1 + (Lv * qs850) / (Rd * T850)
    gamma_den = 1 + (Lv**2 * qs850) / (Cp_d * Rv * T850**2)
    gamma = (g / Cp_d) * (1 - gamma_num / gamma_den)

    EIS = LTS - gamma * (z700 - zlcl)

    EIS = EIS.where(psfc >= p700, 0.0)
    EIS = EIS.where(~np.isnan(EIS), 0.0)
    return EIS.pint.to("K").pint.dequantify()
