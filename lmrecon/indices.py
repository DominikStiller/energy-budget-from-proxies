...
class PatternStabilityIndex(VariabilityIndex):
    """
    Index for tropospheric stability. The index is based on the difference of SSTs in the main
    convective region (which sets the tropospheric temperature) and in the stratus deck regions. It
    is thus an SST-based estimate of the large-scale Estimated Inversion Strength and covaries
    highly with the IPO index.
    """
    ...
    def compute_index(self, da_tos: xr.DataArray) -> xr.DataArray:
        ...
        T_WPWP = area_weighted_mean(da_tos.sel(lat=slice(-20, 20), lon=slice(100, 160)))
        # North/South East Pacific
        T_NEP = area_weighted_mean(da_tos.sel(lat=slice(5, 45), lon=slice(360 - 160, 360 - 120)))
        T_SEP = area_weighted_mean(da_tos.sel(lat=slice(-40, 0), lon=slice(360 - 110, 360 - 70)))
        psi_index = (T_NEP + T_SEP) / 2 - T_WPWP
        return psi_index.rename("psi_index")


class SSTSharpIndex(VariabilityIndex):
    """
    SST#, an index for tropospheric stability. The index is based on the difference of 30% warmest
    tropical SSTs and tropical average SSTs. The 30% warmest SSTs are thought to be related to the
    free-troposphere temperature through deep convection.

    See Fueglistaler (2019; 10.1029/2019GL083990)
    """
    ...
    def compute_index(self, da_tos: xr.DataArray) -> xr.DataArray:
        ...
        Args:
            da_tos: dataset with absolute(!) SSTs
        """
        sst_tropical = da_tos.sel(lat=slice(-30, 30))
        threshold = sst_tropical.quantile(0.7, dim=["lat", "lon"])
        sstsharp = area_weighted_mean(
            sst_tropical.where(sst_tropical >= threshold)
        ) - area_weighted_mean(sst_tropical)
        return sstsharp.rename("sstsharp_index")
