...
class PSM(ABC):
    ...
    def __init__(
        ...
        self.err_std = None
        self.calibration_n_samples = None
        self.calibration_start_year = None
        self.calibration_end_year = None

    @abstractmethod
    def calibrate(self, x: ArrayLike, y: ArrayLike, time: ArrayLike):
        """
        Calibrates the PSM.
        ...
        Args:
            ...
            time: time coordinates of the calibration data ([n_sample])
        ...
        self.calibration_n_samples = y.shape[1]
        self.calibration_start_year = int(time[0])
        self.calibration_end_year = int(time[-1])
...
class LinearPSM(PSM):
    def __init__(
        ...
        lon: float,
        seasonality: None | Season | list[Season],
        regression_model: sklearn.base.BaseEstimator = None,
    ):
        super().__init__(pid, field, lat, lon, seasonality)
        if regression_model:
    ...
    def calibrate(self, x: ArrayLike, y: ArrayLike, time: ArrayLike, year_steps=1):
        x = np.atleast_2d(x)
        y = np.atleast_2d(y)
        ...
        nobs = y.shape[1]
        self.err_std = np.sqrt(ss_res / (nobs - 2))
