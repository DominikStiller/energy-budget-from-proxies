...
from lmrecon.util import get_data_path, remove_empty_proxies, stack_state, to_math_order
...
def run_reconstruction(
    ...
    pdb_assimilated = remove_empty_proxies(pdb_assimilated.slice((year_start - 1, year_end + 2)))
    ...
    with (mapper_path / "metadata.json").open() as f:
        ...
        initial_prior_dataset = Path(mapper_metadata["physical_dataset"]).with_name(
            ...
            # "seasonal_anomalies_with_F_R.zarr"
        )
    ...
    prior = to_math_order(
        stack_state(
            create_initial_ensemble_from_sample(
                ...
                Season.DJF,
                seed_offset,
                # replace=True,  # for certain PPEs where n_ens > len(initial_prior_dataset.time)
            )
        )
    ).compute()
