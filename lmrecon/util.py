...
def stack_state(ds: T_Xarray) -> DataArray:
    ...
    if not (sample_dims := list(set(ds.dims) & {"sample", "time", "ens", "model", "case"})):
        sample_dims = []
...
def get_host() -> str:
    ...
    if hostname == "enkf":
        return "enkf"
    elif hostname.startswith(("casper", "crhtc", "crlogin")):
        return "casper"
...
def is_coordinate_inside_box(lat, lon, lat_bnds, lon_bnds):
    """
    Check if a coordinate is within a latitude-longitude box

    Args:
        lat: Latitude to check
        lon: Longitude to check
        lat_bnds: Latitude bounds of box
        lon_bnds: Longitude bounds of box

    Returns:
        Wther coordinate is within box
    """
    # Check latitude
    lat_inside = lat_bnds[0] < lat < lat_bnds[1]

    # Handle longitude wrapping at 180E
    lon_min, lon_max = lon_bnds

    # Normalize longitude to [0, 360) range
    lon_normalized = lon % 360
    lon_min_normalized = lon_min % 360
    lon_max_normalized = lon_max % 360

    # Check if bounds wrap around the 0/360 line
    if lon_min_normalized > lon_max_normalized:
        # Box wraps around 0/360 (e.g., 350 to 10 becomes 350-360 or 0-10)
        lon_inside = lon_normalized >= lon_min_normalized or lon_normalized < lon_max_normalized
    else:
        # Normal case: no wrapping
        lon_inside = lon_min_normalized < lon_normalized < lon_max_normalized
...
def remove_empty_proxies(pdb: cfr.ProxyDatabase) -> cfr.ProxyDatabase:
    return cfr.ProxyDatabase({pid: pobj for pid, pobj in pdb.records.items() if len(pobj.time) > 0})


class NanMaskXarray:
    def __init__(self, state_dim: str = "state", nan_method="any"):
        self.nan_mask: xr.DataArray | None = None
        self.state_dim = state_dim
        self.index_coords = None
        self.original_coords: xr.Coordinates | None = None
        assert nan_method in ["any", "all"]
        self.nan_method = nan_method

    def _validate_input(self, da: xr.DataArray):
        if self.state_dim not in da.dims:
            raise ValueError(f"state_dim '{self.state_dim}' not found in DataArray dimensions.")

    def fit(self, da: xr.DataArray) -> Self:
        self._validate_input(da)
        sample_dims = [dim for dim in da.dims if dim != self.state_dim]
        self.original_coords = da.coords
        # Need to use integer index coords, otherwise xarray cannot handle MultiIndex in reindex
        self.index_coords = np.arange(len(self.original_coords[self.state_dim]))
        nans = da.assign_coords({self.state_dim: self.index_coords}).isnull()
        self.nan_mask = (
            nans.any(dim=sample_dims) if self.nan_method == "any" else nans.all(dim=sample_dims)
        )
        return self
    ...
    def forward(self, da: xr.DataArray) -> xr.DataArray:
        ...
        if self.nan_mask is None:
            self.fit(da)
        return da.assign_coords({self.state_dim: self.index_coords}).where(
            ~self.nan_mask, drop=True
        )

    def backward(self, da: xr.DataArray) -> xr.DataArray:
        if self.nan_mask is None or self.original_coords is None:
            raise RuntimeError("Must call 'fit' or 'forward' before 'backward'.")

        # Reindex to restore original coordinates, filling missing values with NaN
        return da.reindex({self.state_dim: self.index_coords}).assign_coords(
            {self.state_dim: self.original_coords[self.state_dim]}
        )


class NanMask:
    """
    Masks out nans, which is required for certain computations. Supports multiple sampling dimensions,
    but the state/feature dimension must be the first dimension only.
    """
    ...
    def fit(self, da: ArrayLike) -> Self:
        self.nan_mask = np.array(np.isnan(da).any(axis=tuple(np.arange(1, len(da.shape)))))
        return self
...
def format_minus(s: str) -> str:
    """
    Replace ASCII hyphen-minus with Unicode minus

    Args:
        s: string containing hyphen-minus

    Returns:
        string with Unicode minus
    """
    return s.replace("-", "−")
