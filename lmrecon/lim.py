...
from lmrecon.util import get_timestamp
...
class LIM:
    def __init__(self, rescale_eigenvalues=True):
        self.rescale_eigenvalues = rescale_eigenvalues
        self.tau = None
    ...
    def fit(self, data: xr.DataArray):
        ...
        data_np = data.values  # extract numpy array
        self.Nx = len(self.state_coords)

        if not (data_np.mean(axis=1) < 1e-5).all():
    ...
    def _fit_noise(self, C_0):
        ...
        if num_neg > 0:
            ...
            Q_evals = Q_evals[:-num_neg]
            if self.rescale_eigenvalues:
                logger.info(
                    f"Rescaling {num_left:d} remaining eigenvalues of Q by {self.Q_eval_scale_factor:1.2f}"
                )
                Q_evals = Q_evals * self.Q_eval_scale_factor
            else:
                logger.info(
                    f"Not rescaling {num_left:d} remaining eigenvalues of Q, would have been by {self.Q_eval_scale_factor:1.2f}"
                )
    ...
    def forecast_stochastic(
        ...
        for i in tqdm(range(1, n_int_steps + 1), disable=not progressbar, unit="step"):
            # L * x * dt
