...
from lmrecon.util import get_data_path, list_complement
...
if __name__ == "__main__":
    ...
    if model_id in ["CESM2-WACCM-FV2", "EC-Earth3-Veg-LR", "INM-CM4-8", "ACCESS-ESM1-5"]:
        # Match five-year means before and after stitching point by shifting ds1
        # These models do not have historical runs initialized from past1000
        non_sic_fields = list_complement(set(ds1.data_vars) & set(ds2.data_vars), ["siconc"])
        offset = (
            ds1[non_sic_fields].isel(time=slice(-4 * 5, None)).mean("time")
            - ds2[non_sic_fields].isel(time=slice(None, 4 * 5)).mean("time")
        ).compute()
        ds2[non_sic_fields] = ds2[non_sic_fields] + offset[non_sic_fields]
    ...
    ds.chunk(chunks=dict(time=200)).to_zarr(output_path / "seasonal_averages.zarr")
    ...
    ds_anom.chunk(chunks=dict(time=200)).to_zarr(output_path / "seasonal_anomalies.zarr")
