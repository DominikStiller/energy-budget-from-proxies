...
from lmrecon.time import format_tuple_time
...
def run_forecast(i):
    ...
    print(f"Case {i + 1}/{n_cases} ({format_tuple_time(time)})")
...
if __name__ == "__main__":
    ...
    sample = sorted(
        np.random.default_rng(89155354).choice(
            np.arange(len(data_training.time) - 1 - n_steps), n_cases, replace=False
        )
    )
    ...
    logger.info(f"Computing of verification forecasts for LIM {lim_id} completed")
