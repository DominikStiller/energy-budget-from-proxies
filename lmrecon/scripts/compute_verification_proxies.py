...
def _calculate_proxy_estimate_seasonal(state, psms):
    ...
    seasons = np.array([map_decimal_to_season(t) for t in state.time.values])
    proxy_estimate = np.full(len(state.time), np.nan)
    for season, psm in psms.items():
        mask = seasons == season
        if mask.any():
            proxy_estimate[mask] = np.squeeze(
                psm.forward(np.atleast_2d(state_at_proxy_location.values[mask]))
            )
