...
def generate_pseudoobs_grid(std_noise=1.33, multiplicity=1):
    """
    ...
    for i, (lat, lon) in tqdm(list(enumerate(locations))):
        ...
        tas += noise_rng.normal(0, std_noise, tas.shape)
    ...
    psms = {pid: IdentityPSM("tas", lat, lon, std_noise, None) for pid in records}

    return records, psms, {"std_noise": std_noise, "multiplicity": multiplicity}
...
def _calibrate_pseudo_psm(
    ...
    da_truth_for_proxy = da_truth_for_proxy.dropna("time")
    da_proxy = da_proxy.dropna("time")
    if not constant_replication:
        da_truth_for_proxy, da_proxy = xr.align(da_truth_for_proxy, da_proxy, join="inner")

    if len(da_proxy) == 0 or len(da_truth_for_proxy) == 0:
        # In some cases, the land mask differs between real proxy calibration and the truth data,
        # so all data are nan and there is no overlap. This is only the case for about 7 out of 440
        # proxies, so its okay to ignore that edge case.
...
def generate_pseudoobs_from_real(
    calibrated_obs_dataset: str, scale_snr: float = 1, constant_replication=False
):
    """
    Generate pseudoobs by replicating a real proxy network in terms of seasonality and SNR.

    Args:
        calibrated_obs_dataset: ID of calibrated observation datasets
        scale_snr: Scaling for SNR
        constant_replication: Whether to reproduce availability over time or have constant proxy replication. Seasonality
            is respected in any case.
    ...
    # Only needed when re-calibrating PSMs
    da_truth_truncated = mapper.truncate_dataset(
        xr.merge(
            [
                # Emulate GISTEMP, trimmed to 1900-
...
def generate_pseudoobs_from_real_idealized(calibrated_obs_dataset, std_noise=1, mapper_id=None):
    """
    ...
    for pid, proxy in tqdm(pdb_original.records.items()):
        ...
        if isinstance(psm_original, dict):
            ...
            ppsm = {}
            for season in psm_original:
                pproxy_season, ppsm_season = _generate_identity_psm(
                    # Select like this so that tuple timestamp is retained
                    da_proxy.sel(time=da_proxy.season == season),
                    da_truth_for_proxy.sel(
                        time=da_truth_for_proxy.season == season,
            ...
            pproxy, ppsm = _generate_identity_psm(
                da_proxy,
                da_truth_for_proxy,
                proxy,
                psm_original,
...
if __name__ == "__main__":
    ...
    true_dataset = Path() / "cmip6/CESM2-WACCM-FV2/past1000_historical/seasonal_anomalies.zarr"
    ...
    # true_dataset = Path() / "cmip6/CESM2-WACCM-FV2/piControl/seasonal_anomalies_detrended.zarr"
    # true_dataset = Path() / "cmip6/NorESM2-LM/hist-nat/seasonal_anomalies_with_F_R.zarr"
    # true_dataset = Path() / "cmip6/CanESM5/hist-nat/seasonal_anomalies_with_F_R.zarr"

    logger.info(f"Loading seasonal anomalies from {true_dataset} as truth")
    da_truth = xr.open_zarr(get_data_path() / true_dataset)[["tas", "tos"]].load()

    if "CESM2-WACCM-FV2/piControl" in str(true_dataset):
        # For CESM piControl (years 1-500): shift to 1500-2000
        da_truth = da_truth.assign_coords(time=da_truth["time"] + 1500)

    noise_rng = np.random.default_rng(52565641)

    if type == "real":
        records, psms, metadata = generate_pseudoobs_from_real(
            sys.argv[1], scale_snr=1, constant_replication=False
        )
    elif type == "real_ideal":
        records, psms, metadata = generate_pseudoobs_from_real_idealized(
            sys.argv[1], std_noise=np.sqrt(0.4), mapper_id="2026-02-09T13-19-35"
        )
    elif type.startswith("grid_"):
        records, psms, metadata = generate_pseudoobs_grid(std_noise=np.sqrt(0.4))
    else:
    ...
    logger.info(f"Computing of pseudo-observations (n = {len(records)}) completed")
