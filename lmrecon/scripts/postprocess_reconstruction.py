...
def _compute_regional_means(state, rundir, type):
    ...
    regions = {
        ...
        "60N-90N": (slice(60, None), slice(None, None)),
        "90S-60S": (slice(None, -60), slice(None, None)),
    }
    regions_land = {
        "global_land": (slice(None, None), slice(None, None)),
        "30N-90N_land": (slice(30, None), slice(None, None)),
    }
    ...
    regional_means = xr.concat(
        ...
        + [
            area_weighted_mean(state.where(landmask).sel(lat=lat_bnd, lon=lon_bnd))
            .expand_dims("region")
            .assign_coords(region=[name])
            for name, (lat_bnd, lon_bnd) in regions_land.items()
        ],
        dim="region",
    )
...
def postprocess_reconstruction(rundir):
    host = get_host()
    if host == "enkf":
        dask.config.set({"temporary_directory": "/home/enkf6/dstiller/tmp"})
        client = Client(n_workers=2, threads_per_worker=1)  # noqa: F841
