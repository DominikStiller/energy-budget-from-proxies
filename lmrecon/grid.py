...
def regrid_hybrid_to_pressure(
    ...
    ds = ds.assign_coords(lev=plev)

    return regrid_pressure(ds=ds, field=field, lev_target=lev_target)
...
def regrid_pressure(
    ...
    da_pressure = grid.transform(
        ds[field], "lev", np.array(lev_target), target_data=ds["lev"], method="log"
    )
